...
echo '<div class="contents">
   <div class="sub_title">EVENTS</div>
...
if (isset($AddonsRoot->getParams($HOST)["reference"]) && is_numeric($AddonsRoot->getParams($HOST)["reference"])) {
  $View = $FeaturesNews->viewNotice(2, $AddonsRoot->getParams($HOST)["reference"]);
  if ($View[1] <= 0) {
  ...
  $FeaturesNews->eventList($AddonsRoot->getPage($HOST));
  echo '</ul>
<ul class="bbs_paging">';
 
  $FeaturesNews->noticesPages($AddonsRoot->getParams($HOST), 2);
  echo '</ul>
</div>';
}
echo '</div></div>';
