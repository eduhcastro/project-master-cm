...
echo '<div class="contents">
   <div class="sub_title">PATCH NOTE</div>
...
if (isset($AddonsRoot->getParams($HOST)["reference"]) && is_numeric($AddonsRoot->getParams($HOST)["reference"])) {
  $View = $FeaturesNews->viewNotice(1, $AddonsRoot->getParams($HOST)["reference"]);
  if ($View[1] <= 0) {
  ...
  $FeaturesNews->patchList($AddonsRoot->getPage($HOST));
  echo '</ul>
<ul class="bbs_paging">';
 
  $FeaturesNews->noticesPages($AddonsRoot->getParams($HOST), 1);
  echo '</ul>
</div>';
}
echo '</div></div>';
